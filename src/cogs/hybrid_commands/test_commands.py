...
class TestCommandsHybrid(commands.Cog):
    ...
    @commands.hybrid_command(name="hybrid-test", description = "Test single argument input")
    ...
    @commands.hybrid_command(name="hybrid-test2", description = "Test multiple argument input as a keyword")
    async def test_keyword(self, ctx, *, arg):  # takes arguments as one big line of text
        if not check_master_user(ctx.author.id):
            await send_message_not_admin(ctx)
            return
