...
class YesNoView(discord.ui.View):
    ...
    def remove_user_from_string_list(self, list_string: str, user: discord.User):
        ...
        filtered_list = [user_string for user_string in string_list if user.name not in user_string]
        return "\n".join(filtered_list)

    @discord.ui.button(label="Yes", style=discord.ButtonStyle.green, emoji="👍🏼") # or .success
        ...
        for field in embed.fields:
            if field.name == "---No---":
                ...
                if interaction.user.name in field.value:
                    ...
                    new_value = self.remove_user_from_string_list(new_value, interaction.user)
                    embed.set_field_at(embed.fields.index(field), name="---No---", value=new_value, inline=True)
            ...
            if field.name == "---Yes---":
                ...
                if interaction.user.name in field.value:
                    ...
                    new_value = self.add_number_to_string(field.value, 1)
                    new_value = self.add_user_to_string_list(new_value, interaction.user)
                
                embed.set_field_at(embed.fields.index(field), name="---Yes---", value=new_value, inline=True)
    
        await interaction.response.edit_message(embed=embed, view=self)

        

    @discord.ui.button(label="No", style=discord.ButtonStyle.red, emoji="👎🏼") # or .danger
